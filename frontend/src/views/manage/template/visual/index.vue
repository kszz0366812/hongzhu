  ...
  <div class="template-config">
    <el-card>
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="tepName" label="模板名称" />
        <el-table-column prop="itfName" label="接口名称" />
        ...
        <el-table-column prop="isPage" label="是否分页">
          <template #default="scope">
...
import { getCustomInterfacePage,getTemplateList} from '@/api/template'
...
const fetchData = async () => {
  loading.value = true
  try {
    // 取第一个模板的 itf_id 作为示例，实际可根据业务调整
    const page = pagination.current
    const size = pagination.pageSize
    // 这里假设不需要额外参数，如有需要可补充
    const res = await getCustomInterfacePage(1, page, size, {})
    // 适配后端返回结构
