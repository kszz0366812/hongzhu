  ...
  <div class="interface-config">
    <el-card>
      ...
      <el-table
        ...
        border
      >
        <el-table-column prop="tepName" label="接口名称" />
        <el-table-column prop="repType" label="接口类型">
          <template #default="{ row }">
            {{ row.repType === 1 ? '查询接口' : row.repType === 2 ? '新增接口' : row.repType === 3 ? '修改接口' : row.repType === 4 ? '删除接口' : '' }}
          </template>
        </el-table-column>
        <el-table-column v-if="tableData.some(row => row.repType === 1)" prop="isPage" label="是否分页">
          <template #default="{ row }">
...
const fetchData = async () => {
  loading.value = true;
  try {
    const res = await getTemplateListPage(pagination.current,pagination.pageSize,{tepName: searchForm.tepName});
