...
public class DataDictionaryServiceImpl extends ServiceImpl<DataDictionaryMapper, DataDictionary> implements DataDictionaryService {
    ...
    @Override
    public Page<DictionaryVO> getPage(Integer pageNum, Integer pageSize, String name, String code, String type, Long parentId, String parentName) {
        // 先检查父级名称查询是否会导致空结果
        if (StringUtils.hasText(parentName)) {
            ...
            if (parentIds.isEmpty()) {
                // 没有匹配的父级，直接返回空分页
                Page<DictionaryVO> emptyPage = new Page<>();
        ...
        Set<Long> parentIds = dataList.stream()
        ...
        // 转换为VO
        List<DictionaryVO> voList = dataList.stream()
                .map(dict -> convertToVO(dict, idNameMap))
                .collect(Collectors.toList());
        
        // 构建返回的Page对象
        Page<DictionaryVO> voPage = new Page<>();
        voPage.setRecords(voList);
        voPage.setTotal(pageInfo.getTotal());
        voPage.setCurrent(pageInfo.getPageNum());
        voPage.setSize(pageInfo.getPageSize());
