...
public class ExcelServiceImpl implements ExcelService {
    ...
    private <T extends ExcelDTO> void insertDataToDatabase(List<T> successList) {
        ...
        try {
            ...
            // 获取save方法
            Method saveMethod = findSaveMethod(service.getClass());
            if (saveMethod == null) {
                log.error("未找到Service的save方法: {}", serviceClass.getSimpleName());
                return;
            }

            log.info("开始插入数据到数据库，数据类型: {}，数据量: {}条", 
            ...
            for (T dto : successList) {
                try {
                    ...
                    // 调用Service的save方法
                    Object result = saveMethod.invoke(service, entity);
                    ...
                    failCount++;
                    log.error("插入数据时发生错误: {}", e.getMessage(), e);
                }
            }

            log.info("数据库插入完成，成功: {}条，失败: {}条", successCount, failCount);
     ...
     * @return save方法，如果未找到返回null
     */
    private Method findSaveMethod(Class<?> serviceClass) {
        // 优先查找save方法
        try {
            return serviceClass.getMethod("save", Object.class);
        } catch (NoSuchMethodException e) {
            // 如果没有save方法，尝试其他常见的方法名
            String[] methodNames = {"add", "insert", "create"};
    ...
    public <T extends ExcelDTO> void downloadTemplate(HttpServletResponse response, Class<T> dtoClass) {
        try {
            ...
            String sheetName = importType.getFileName();
