...
public class CustomizeController {
     ...
     * @return
     */
    @RequestMapping("/getInterfaceInfoPage/{id}/{page}/{size}")
    public Result<Object> getInterfaceInfoPage(@PathVariable Integer id, @PathVariable Integer page,  @PathVariable Integer size,@RequestBody JSONObject param) {
        ItfTemplate template = itfTemplateService.getById(id);
        if (template == null) {
            return Result.error("接口不存在");
        }
        String resourceSql = template.getResourceSql();
        String sql  = SqlConvert.convert(resourceSql, param);
        IPage<Object> result = customService.getCustomizeSqlPage(sql, page, size);
