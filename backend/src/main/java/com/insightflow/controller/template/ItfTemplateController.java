...
public class ItfTemplateController {
    ...
    public Result<Void> deleteTemplate(@Parameter(description = "模板ID") @PathVariable Integer id) {
        itfTemplateService.deleteTemplate(id);
        return Result.success();
    }

    @Operation(summary = "分页查询接口模板", description = "分页查询接口模板列表，支持按模板名称搜索")
    @RequestMapping("/page/{pageNum}/{pageSize}")
    public Result<Page<ItfTemplate>> pageTemplate(
            @Parameter(description = "页码") @RequestParam(defaultValue = "1") Integer pageNum,
            @Parameter(description = "每页大小") @RequestParam(defaultValue = "10") Integer pageSize,
            @RequestBody ItfTemplateDTO dto) {
        return Result.success(itfTemplateService.pageTemplate(pageNum, pageSize, dto.getTepName()));
    }
